...
class ViewMonths extends JPanel {
	private static final long serialVersionUID = -6419560074058515943L;
	...
	public ViewMonths(Calendar initialDate, Calendar minDate, Calendar maxDate) {
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseReleased(MouseEvent me) {
				ViewMonths.this.firePropertyChange("Confirm", null, selectedDate);
			}
		});
		setTableSelection();
	...
	private JPanel YearMonthSpinner() {
		...
		monthSpinner = new JSpinner(monthModel) {
			private static final long serialVersionUID = 8096301049597505266L;
		...
		yearSpinner = new JSpinner(yearModel) {
			private static final long serialVersionUID = 4724196177398930822L;
			public ComponentOrientation getComponentOrientation() {
	...
	private void changeTableAction(int key) {
			...
			replacement = new AbstractAction() {
				public void actionPerformed(ActionEvent ae) {
					...
					if (table.getSelectedColumn() == 0) {
						...
						ViewMonths.this.firePropertyChange("Value", selectedDate, newValue);
						setValue(newValue);
						allow = false;
					}
					if (allow) {
		...
		case KeyEvent.VK_UP:
			replacement = new AbstractAction() {
				public void actionPerformed(ActionEvent ae) {
					boolean allow = true;
					if (min != null && selectedDate.getTimeInMillis() - min.getTimeInMillis() < ONE_DAY * 7) {
						ViewMonths.this.firePropertyChange("Value", selectedDate, min);
						setValue(min);
						allow = false;
					}
		...
		case KeyEvent.VK_DOWN:
			replacement = new AbstractAction() {
				public void actionPerformed(ActionEvent ae) {
					boolean allow = true;
					if (max != null && max.getTimeInMillis() - selectedDate.getTimeInMillis() < ONE_DAY * 7) {
						ViewMonths.this.firePropertyChange("Value", selectedDate, max);
						setValue(max);
						allow = false;
					}
	...
	private void setTableSelection() {
		boolean previousChange = programmaticChange;
		programmaticChange = true;
		int days = (int)((selectedDate.getTimeInMillis() - startDate.getTimeInMillis()) / ONE_DAY);
	...
	private class MonthViewTableModel<T extends Calendar> extends AbstractTableModel {
		private static final long serialVersionUID = 986182075243658992L;
	...
	private class MonthViewTableCellRenderer extends DefaultTableCellRenderer {
		private static final long serialVersionUID = 1775738262251385170L;
